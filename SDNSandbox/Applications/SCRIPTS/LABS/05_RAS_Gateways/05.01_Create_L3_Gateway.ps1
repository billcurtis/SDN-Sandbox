...
.SYNOPSIS 
    ...
    This script:
     ...
     1. This script should be run on the Console VM with the user logged on as Domain\Administrator 
...
#>
    ...
    Invoke-Command -ComputerName Admincenter -ScriptBlock {

        $params = @{

            Name           = 'L3Connection'
            LocalIPAddress = $using:extBGPIP
            PeerIPAddress  = $using:bgpRouterID
            LocalASN       = $using:extBGPASN
            PeerASN        = $using:intBGPASN
            OperationMode  = 'Mixed'
